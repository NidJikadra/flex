<?php
/**
 * Case-studies-list Section Component
 */
?>

<section class="case-studies-list">
    <div class="container">
        <div class="section-title">
            <div class="title-wrapper">
                <p class="sub-title">Lorem ipsum</p>
                <h2 class="h2">Spotlight on Our Design Work</h2>
                <div class="description">Explore our portfolio to discover a diverse range of design projects that reflect our passion for crafting visually stunning and user-centric solutions.</div>
            </div>
        </div>
        <div class="case-studies-list__content">
            <div class="case-studies-list__content-list">
                <div class="case-studies-list__image-container">
                    <img src="assets/images/case-studies1.png" width="690" height="781"
                        alt="Laptop with virtual dictionary" class="case-studies-list__laptop-image">
                </div>
                <div class="case-studies-list__details">
                    <p class="tech-field">
                        <span>UI</span>
                        <span>UX</span>
                        <span>Branding</span>
                        <span>Web Design</span>
                        <span>Development</span>
                        <span>3D Design</span>
                    </p>
                    <h3 class="h3">The Interactive Dictionary of Free Speech - WebExperience</h3>
                    <p class="overview-text">
                        A virtual reality gaming experience showcasing the wonders of the island of Nisida, intertwining
                        myths, legends, and tourist information with a message of hope and redemption.
                    </p>
                    <div class="case-studies-list__technologies">
                        <div class="case-studies-list__tech">
                            <div class="grid-image">
                                <img src="assets/images/icons/figma-case.svg" height="45" width="45" alt="Figma">
                            </div>
                            <div class="item-name">Figma</div>
                        </div>
                        <div class="case-studies-list__tech">
                            <div class="grid-image">
                                <img src="assets/images/icons/angular-case.svg" height="45" width="45" alt="Angular">
                            </div>
                            <div class="item-name">Angular</div>
                        </div>
                        <div class="case-studies-list__tech">
                            <div class="grid-image">
                                <img src="assets/images/icons/js-case.svg" height="45" width="45" alt="JavaScript">
                            </div>
                            <div class="item-name">JavaScript</div>
                        </div>
                        <div class="case-studies-list__tech">
                            <div class="grid-image">
                                <img src="assets/images/icons/react-case.svg" height="45" width="45" alt="React">
                            </div>
                            <div class="item-name">React</div>
                        </div>
                    </div>
                    <div class="case-studies-list__stats">
                        <div class="case-studies-list__stat">
                            <p class="case-studies-list__stat-value">~25%</p>
                            <p class="case-studies-list__stat-description">Navigate effortlessly with our intuitive
                                interface.</p>
                        </div>
                        <div class="case-studies-list__stat">
                            <p class="case-studies-list__stat-value">1m</p>
                            <p class="case-studies-list__stat-description">Enjoy reliable, cutting-edge technology every
                                time.</p>
                        </div>
                    </div>
                    <button type="button" class="button btn-secondary">Explore the Details<span
                            class="icon"></span></button>
                </div>
            </div>
        </div>
        <button type="button" class="button btn-primary bottom">See All Case Studies</button>
    </div>
</section>