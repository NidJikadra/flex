<?php
/**
 * Core Values Section Component
 */
?>

<section class="core-values">
    <div class="container">
        <div class="section-title">
            <div class="title-wrapper">
                <p class="sub-title">Our Core Values</p>
                <h2 class="h2">Principles that Shape Our Vision</h2>
            </div>
        </div>
        <div class="core-values-wrapper">
            <div class="core-values__value" data-image="image-1" data-animate="fade-up" data-animate-delay="0">
                <div class="item">
                    <span class="number text-blue">01</span>
                    <div class="text">
                        <h3 class="h3">Innovation</h3>
                        <p class="description">We are committed to pushing the boundaries of what’s possible, constantly
                            exploring new frontiers in AI and Web3 technologies.</p>
                    </div>
                </div>
                <div class="core-values__dynamic-image">
                    <img id="image-1" src="assets/images/innovation.png" width="309" height="209" alt="Innovation">
                </div>
            </div>
            <div class="core-values__value active" data-image="image-2" data-animate="fade-up" data-animate-delay="0.1">
                <div class="item">
                    <span class="number text-blue">02</span>
                    <div class="text">
                        <h3 class="h3">Integrity</h3>
                        <p class="description">Honesty and transparency are at the heart of our operations, ensuring
                            trust and reliability in everything we do.</p>
                    </div>
                </div>
                <div class="core-values__dynamic-image">
                    <img id="image-2" src="assets/images/innovation.png" width="309" height="209" alt="Integrity"
                        class="active">
                </div>
            </div>
            <div class="core-values__value" data-image="image-3" data-animate="fade-up" data-animate-delay="0.2">
                <div class="item">
                    <span class="number text-blue">03</span>
                    <div class="text">
                        <h3 class="h3">Impact</h3>
                        <p class="description">Our focus is on making a meaningful difference, leveraging technology to
                            address real-world challenges and improve lives.</p>
                    </div>
                </div>
                <div class="core-values__dynamic-image">
                    <img id="image-3" src="assets/images/innovation.png" width="309" height="209" alt="Impact">
                </div>
            </div>
            <div class="core-values__value" data-image="image-4" data-animate="fade-up" data-animate-delay="0.3">
                <div class="item">
                    <span class="number text-blue">04</span>
                    <div class="text">
                        <h3 class="h3">Collaboration</h3>
                        <p class="description">We believe in the power of teamwork, fostering a culture where diverse
                            ideas come together to create superior solutions.</p>
                    </div>
                </div>
                <div class="core-values__dynamic-image">
                    <img id="image-4" src="assets/images/innovation.png" width="309" height="209" alt="Collaboration">
                </div>
            </div>
        </div>
    </div>
</section>